...
def get_term_egy_diff(energies, nistenergies):
    print("TEST",energies['2J','S'])
    compare = energies.merge(nistenergies, on=['2J','S','L','CF'])
    print("COMPARE COLUMNS",compare.columns)
    print("ENERGIES",compare['CF'])
    termenergies = compare['CF','ENERGY(RYD)','NIST(RYD)'].groupby(by='CF').mean()
    print("TERMENERGIES",termenergies)
