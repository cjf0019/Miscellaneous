...
def ellipsoidvolume(x, y, a, b, c):
    sqrt = (1-float(x)**2/float(a)**2-float(y)**2/float(b)**2)
    if sqrt < 0:
        return float(0)  #prevents nan
    else:
...
def ellipsoidx2(x, y, a, b, c):
    f = 2 * (x**2) * ellipsoidvolume(x, y, a, b, c)   #the two comes from the symmetry of x and y (x**2 + y**2 == x**2 + x**2)
    return f
    ...
    for i in range(step):
        ...
        for j in range(step):
            ...
            if np.isnan(f(x1, y1)):
                ...
                integral += f(x1, y1) * hx * hy   #multiple value at midpoint by dx and dy
    return integral
